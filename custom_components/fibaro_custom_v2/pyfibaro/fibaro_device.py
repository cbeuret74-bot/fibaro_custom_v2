...
class DeviceModel:
    ...
    @property
    def uiCallbacks(self) -> list[str]:
        """Get the available uiCallback names."""
        ui_callbacks = self.properties.get("uiCallbacks") or []
        return [callback.get("callback") for callback in ui_callbacks]
    ...
    def execute_callAction(
        self, callAction: str, arguments: list[Any] | None = None
    ) -> Any:
        """Execute a device callAction (uiCallback).
        ...
        if callAction not in self.uiCallbacks and callAction not in self.actions:
            _LOGGER.warning(
                "The device %s has no action %s. Possible callbacks: %s or %s",
                self.fibaro_id,
                callAction,
                self.uiCallbacks,
                self.actions,
        ...
        for index, arg in enumerate(arguments or [], start=1):
            url += f"&arg{index}={arg}"
        _LOGGER.debug(
            "Execute %s for device %s url %s", callAction, self.fibaro_id, url
        )
