...
# Modes du com.fibaro.hvacSystemHeat (libellés français du HC3)
HEATMODES = {
    "Off": "off",
    "Eco": "heat",
    "Hors-Gel": "cool",
...
HA_HEATMODES = {v: k for k, v in HEATMODES.items()}
...
class FibaroThermostat(FibaroEntity, ClimateEntity):
                ...
                for mode in device.supported_thermostat_modes:
                    try:
                        if device.type != "com.fibaro.hvacSystemHeat":
                            ...
                            if mode not in HEATMODES:
                                _LOGGER.warning("%s unknown heat mode", mode)
                                continue
                            self._attr_hvac_modes.append(HVACMode(HEATMODES[mode]))
                    except ValueError:
    ...
    def fibaro_op_mode(self) -> str | int:
        ...
        if device.has_thermostat_mode:
            ...
            return HEATMODES.get(device.thermostat_mode, device.thermostat_mode)
        return device.mode
                ...
                for mode in device.supported_thermostat_modes:
                    if device.type != "com.fibaro.hvacSystemHeat":
                        if mode.lower() == hvac_mode:
                            ...
                            break
                    elif mode == HA_HEATMODES.get(hvac_mode):
                        device.execute_action("setThermostatMode", [mode])
                        break
